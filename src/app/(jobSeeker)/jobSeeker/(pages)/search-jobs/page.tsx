...
export default function SearchJobPage() {
  ...
  useEffect(() => {
    setSearchQuery(searchParams.get("jobName"));
    setLocationQuery(searchParams.get("location"));
    setKeywordQuery(searchParams.get("searchTerm"));
  }, [searchParams]);
        ...
        {searchQuery && locationQuery ? (
          ...
          <div className="flex gap-4 items-center">
            ...
            <p className="md:text-xl bg-gray-200 px-5 py-1 rounded-full flex gap-3 items-center">
              {keywordQuery}
              <button
                onClick={() => handleCross("searchTerm")}
                className="mt-1 text-sm cursor-pointer"
              >
                ✕
              </button>
  ...
  `}
        >
          <FilterSidebar
            setIsFilterSidebarVisible={setIsFilterSidebarVisible}
